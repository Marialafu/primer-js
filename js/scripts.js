...
function totalApples(apples, box){
    const result= apples/box
    console.log(result + ' manzanas tendrá cada caja');
}
totalApples(240, 8)

//6️⃣ Bego tiene 57 tornillos y quiere guardarlos en cajitas de 10 tornillos cada una. ¿Cuántos tornillos quedarán fuera?

function tornillosPorCaja(tornillos, box){
    const result= tornillos%box
    console.log(result + ' tornillos quedarán fuera');
}
tornillosPorCaja(57, 10)

//7️⃣ Abby cobra 35 euros por misión. Si hizo 5 misiones en un día, ¿cuánto dinero ganó?

function ejercicio7(euros, misions){
    const result= euros*misions
    console.log(result + ' eurazos ganará Abby');
}
ejercicio7(35, 5)
...
function moneyPerCount (euros, counts){
    const result = euros / counts
    console.log(result + ' dinero en cada cuenta');
}
moneyPerCount(30000, 4)
...
function cookiesPerDog (cookies, dog){
    const result = cookies / dog
    console.log(result + ' galletas por perro');
}
cookiesPerDog(72, 9)

//2️⃣2️⃣ Abby quiere comprar 5 cuchillos. Si cada uno cuesta 18 euros, ¿cuánto pagará en total?

function moneyForCuchillo (cuchillos, euros){
    const result = cuchillos * euros
    console.log(result + ' euros costó el cuchillo');
}
moneyForCuchillo(5, 18)
...
function flourLeft(perServiceGrams, totalGrams){
    const result = totalGrams / perServiceGrams
    console.log(result + ' g de harina sobrarán');
}
flourLeft(24, 500)